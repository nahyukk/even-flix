...
const MoviePage: React.FC<MoviePageProps> = ({type}) => {
    const [forYou, setForYou] = useState<MoviePageProps[]>([]);
    const [todayTop10SeriesKR, setTodayTop10SeriesKR] = useState<MoviePageProps[]>([]);
    const [realitySeries, setRealitySeries] = useState<MoviePageProps[]>([]);
    const [topRatedSeries, setTopRatedSeries] = useState<MoviePageProps[]>([]);
    const [englishContents, setEnglishContents] = useState<MoviePageProps[]>([]);
    const [uSMovies, setUSMovies] = useState<MoviePageProps[]>([]);
    const [koreanSeries, setKoreanSeries] = useState<MoviePageProps[]>([]);
    const [popularMovies, setPopularMovies] = useState<MoviePageProps[]>([]);
    const [nowPlayingMovies, setNowPlayingMovies] = useState<MoviePageProps[]>([]);
    const [sFAndFantasySeries, setSFAndFantasySeries] = useState<MoviePageProps[]>([]);
    ...
    const fetchHome = async () => {
      try {
        ...
        const todayTop10SeriesDataKR = todayTop10SeriesData.filter(
          (item: MoviePageProps) => item.origin_country?.includes("KR")
        );

        let todayTop10SeriesFinalKR: MoviePageProps[] = [];
